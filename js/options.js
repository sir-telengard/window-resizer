document.querySelector('#add').addEventListener('click', function () {
    ...
    if (width && height) {
        ...
        if (left) newItem.left = parseInt(left, 10);
        if (top) newItem.top = parseInt(top, 10);
...
function renderPresets() {
    ...
    list.forEach(function (obj, index) {
        ...
        const label = document.createTextNode(formatItem(obj));
    ...
    document.querySelectorAll('.edit').forEach(function (ele) {
        ele.addEventListener('click', function () {
            ...
            update.querySelectorAll('[disabled]').forEach(ele => ele.removeAttribute('disabled'));
            var index = parseInt(this.getAttribute('data-index'), 10);
            var item = getList(storageName)[index];
